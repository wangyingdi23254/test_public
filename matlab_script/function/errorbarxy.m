...
for k=1:length(x)
    if ~isempty(lx) & ~isempty(ly) % both errors are specified
        ...
        hold on;
        l2=line([lx(k) lx(k)],[y(k)-0.2*erry(k) y(k)+0.2*erry(k)]);
        l3=line([ux(k) ux(k)],[y(k)-0.2*erry(k) y(k)+0.2*erry(k)]);
        l4=line([x(k) x(k)],[ly(k) uy(k)]);
        l5=line([x(k)-0.2*errx(k) x(k)+0.2*errx(k)],[ly(k) ly(k)]);
        l6=line([x(k)-0.2*errx(k) x(k)+0.2*errx(k)],[uy(k) uy(k)]);
