...
This file consits of two columns:Chamber Pressure & Spark signal
...
dataFilename = '20171109_Test_2.txt';
rcm.initial.P =0.4035;     % bar
rcm.initial.T = 273.15+23.2;     % K
rcm.config = '0';          % mm
...
rcm.lookup.ign.first_lower = 0.5705 *10^6;   %Input LOWER limit for looking up Fisrt ign [only if first stage happens]
rcm.lookup.ign.first_upper = 0.5722 *10^6;   %Input UPPER limit for looking up Fisrt ign [only if first stage happens]
...
rcm.P.Filter.NyquistFreq = 10^6/2;
...
Mole_sum = rcm.Molar.O2 + rcm.Molar.N2 + rcm.Molar.Ar + rcm.Molar.H2 + rcm.Molar.CO2 + ...
    rcm.Molar.CO + rcm.Molar.H2O + ...
        ...
        + rcm.Molar.MCH + rcm.Molar.Isobutanol +rcm.Molar.CH4 +rcm.Molar.CH3OH+rcm.Molar.nButane; 
...
rcm.P.post = ( (rcm.raw.data(:,N_COLUMN)-...
    sum(rcm.raw.data(1:200000,N_COLUMN)/200000))*rcm.P.AMP ) + rcm.initial.P;
...
rcm.lookup.TDC.zero_time = rcm.lookup.TDC.zero_index/10^3; 
                                % Scale in [ms]
...
rcm.ign.first_time = ( rcm.lookup.ign.first_index - rcm.lookup.TDC.zero_index +1)/1000;

[rcm.lookup.ign.total_flag, rcm.lookup.ign.total_index] = max(...
    rcm.P.dP( rcm.lookup.ign.total_lower+1:rcm.lookup.ign_upper));

[rcm.lookup.ign.summit, rcm.lookup.ign.summit_index] = max(...
    rcm.P.post( rcm.lookup.ign.total_lower+1:rcm.lookup.ign_upper));

rcm.lookup.ign.total_index = rcm.lookup.ign.total_index + rcm.lookup.ign.total_lower;

rcm.lookup.ign.summit_index = rcm.lookup.ign.summit_index + rcm.lookup.ign.total_lower;

rcm.ign.total_time = (rcm.lookup.ign.total_index - rcm.lookup.TDC.zero_index+1)/1000;
...
if (max(abs(rcm.spark))<1)
    rcm.lookup.spark_index = 0;
else
    rcm.lookup.spark_index = find(rcm.spark<-2, 1 ,'last');
end

rcm.ign.spark_time = ( rcm.lookup.spark_index - rcm.lookup.TDC.zero_index +1)/1000;
...
while dP_sum < log( rcm.P_ratio )
    ...
    Cp=(H2*rcm.Molar.H2 + N2*rcm.Molar.N2 + O2*rcm.Molar.O2 + Ar*rcm.Molar.Ar +...
        ...
        Isobutanol*rcm.Molar.Isobutanol +CH3OH*rcm.Molar.CH3OH +n_Butane*rcm.Molar.nButane)*R;
    Cv = Cp - R;
    gamma = Cp / Cv;
...
plot( [rcm.lookup.TDC.zero_time rcm.lookup.TDC.zero_time]./1E3,...
    [min( rcm.P.post ) max(rcm.P.post)] ,'-.b');
...
plot( [rcm.lookup.TDC.zero_time+rcm.ign.spark_time ...
    ...
    [min( rcm.P.post ) max(rcm.P.post)] ,'-.r');
                                        % Plot spark
hold all;
...
str = [ 'Peff = ' num2str(rcm.P_eff ,'%.2f')  '  Teff = ' num2str(rcm.T_eff,'%.2f') ...
    ...
    '   Pmax = ' num2str( rcm.P.post(rcm.lookup.ign.summit_index),'%.2f' )] ;
text('Position',[0,rcm.P.post(rcm.lookup.ign.summit_index)/2],...
    'String',str,...
